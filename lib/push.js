...
process.on('message', function (msg) {
	...
	var InsightKeenIo = new InsightKeenIo(msg);
	var config = InsightKeenIo.config;
	...
	async.forEachSeries(Object.keys(queue), function (cb) {
		...
		var options = InsightKeenIo._getRequestObj(eventCollection, analyticsData);
		request(options, function (err, res, body) {
			if (res) console.log('Success: ', body);
			if (err) {
				cb(err);
				return;
			}
